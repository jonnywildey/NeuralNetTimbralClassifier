...
public class StringWriter {
    ...
    public StringWriter(String name, String stringToWrite) {
    	this(name);
    	this.stringToWrite = stringToWrite;
    }
    
	public StringWriter(final String name) {
	    filename = name ;
	    try {
	...
	/** main string writing method */
	public void writeTheString(String file) {
		try {
		writer.write(file);
		} catch (IOException e) {
			error("Couldn't write string to file " + filename);
		}
		
	}
	
	public void writeTheString() {
		try {
			writeTheString(stringToWrite);
		} catch (Exception e) {
			error("did not pass string to write");
		}
	}
