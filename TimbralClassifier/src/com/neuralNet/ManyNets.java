...
public class ManyNets implements Callable<MultiLayerNet[][]> {
	 ...
	 */
	public static MultiLayerNet[] runNets(int runCount,
		...
		for (int i = 0; i < runCount; ++i) {
			...
			nns[i] = RunNetwork.config(new MultiLayerNet(), testPatterns,
					neuronCount, verbose, seed, seed2); // Make a net;
			try {
				nns[i].runEpoch();
	...
	public MultiLayerNet[][] call() {
		...
		MultiLayerNet[][] nets = tryDifferentLayers(runCount, testPatterns,
				verbose, even);
		// CoefficientLogger[][] cls =
		// CoefficientLogger.getErrorsFromMultiLayer(nets);
	 ...
	 * @return the multi layer net[][]
	 */
	public MultiLayerNet[][] tryDifferentLayers(int runCount,
			TestPatterns testPatterns, boolean verbose, boolean even) {
		int[] layerOneSize = new int[] { 1, 5, 10, 15, 20, 30, 40, 50, 60, 70,
				80, 90, 100, 110, 120, 150, 160, 200 };
		MultiLayerNet[][] mlns = new MultiLayerNet[layerOneSize.length / 2][runCount];
		if (even) {
			for (int i = 0; i < mlns.length; i++) {
				mlns[i] = runNets(runCount, testPatterns,
						layerOneSize[(i * 2)], verbose);
			}
		} else {
			for (int i = 0; i < mlns.length; i++) {
				mlns[i] = runNets(runCount, testPatterns,
						layerOneSize[(i * 2) + 1], verbose);
			}
		}
		return mlns;
	}
