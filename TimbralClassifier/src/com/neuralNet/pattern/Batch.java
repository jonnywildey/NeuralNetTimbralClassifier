...
public class Batch {
	 ...
	 *            the mono
	 */
	protected static void batchWavProcess(File sampleDir, File newDir,
			boolean mono) {
		...
		for (File nf : files) {
			for (int i = 0; i < 15; ++i) {
				...
				s1 = SignalChain.processSignalChain(pitchRand, noiseRand,
						hpRand, lpRand, s1);
				FFTBox fftBox = null;
				if (mono) {
				...
				Wave newWave = WavePattern.addMetaData(s1, fftBox, "IAS8",
						instrument);
	...
	/**
	 * Performs batch processing of all the audio files in a directory. For
	 * monotimbral processing *
	 * 
	 * @param sampleDir
	 *            the sample dir
	 * @param newDir
