...
public class WavePatternsRegenRewrite extends WavePatterns {
	...
	/**
	 * turns a set of waves to a pattern. by using FFT analysis (input
	 * analysis). Then rewrites data chunks of Wave
	 **/
	public void wavRewrite(File[] files) {
		...
		for (int i = 0; i < files.length; ++i) {
			...
			patterns[i].setInputArray(Pattern.doubleToInputShell(fftBox
					.getValues()[0]));
			...
			WavePattern.addMetaData(signal, fftBox, "IAS8",
					patterns[i].instrument);
		}
		this.instrs = instrs;
		getOutputs(instrs);
	}
