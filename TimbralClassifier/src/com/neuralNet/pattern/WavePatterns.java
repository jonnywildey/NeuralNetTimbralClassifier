...
/**
 * Container object for multiple Wave Patterns. Allows for multithreaded
 * generation of patterns from a folder of Waves, regeneration of patterns from
 * metadata in waves, batch processing and some other useful methods. See sub
 * classes for other multi threaded operations
 **/
public class WavePatterns implements Serializable, Callable<WavePatterns> {
	...
	protected static void runJobs(int threadCount, WavePatterns[] newWP) {
		...
		Set<Future<WavePatterns>> futures = new HashSet<Future<WavePatterns>>(
				threadCount);
		for (int i = 0; i < threadCount; ++i) {
	...
	public static WavePattern[][] splitPatterns(WavePattern[] patterns,
			int count) {
		...
		for (int i = 0; i < this.patterns.length; ++i) {
			for (int j = 0; j < targets.length; ++j) {
				if (instrs[i].equals(targets[j][0])) {
					patterns[i].setTargetArray(ArrayMethods
							.doubleToArrayList(bits[j]));
	...
	/**
	 * Reduces the size of all patterns input by 2^x. Useful if amplitudes were
	 * not normalised
	 **/
	...
	/**
	 * Set patterns. Converts arraylist to array, so be careful with large
	 * arraylists
	 */
