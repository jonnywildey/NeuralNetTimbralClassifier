...
public class MultiNNUtilities {
	...
	/**
	 * MultiThreaded committee generation. generates a thread for every single
	 * run count, so be careful with very large committees *
	 * 
	 * @param testPatterns
	 *            the test patterns
	 * @param runCount
	...
	public static Committee createCommittee(TestPatterns testPatterns,
			int runCount, int hiddenLayer, int epoch, boolean verbose) {
		Committee committee = new Committee();
		ThreadNet[] threadNets = MultiNNUtilities.generateThreadNets(runCount,
				testPatterns, hiddenLayer, epoch, verbose);
		ArrayList<MultiLayerNet> aNets = runCallableThreads(runCount,
				threadNets, MultiLayerNet.class);
	 ...
	 */
	public static ThreadNet[] generateThreadNets(int runCount,
			TestPatterns testPatterns, int hiddenLayer, int epoch,
			boolean verbose) {
		ThreadNet[] manyNets = new ThreadNet[runCount];
		for (int i = 0; i < runCount; ++i) {
			manyNets[i] = new ThreadNet(null, testPatterns, hiddenLayer, epoch,
					i, verbose);
		}
		return manyNets;
	}
	 ...
	 */
	public static <T, V extends Callable<T>> ArrayList<T> runCallableThreads(
			int count, V[] thingsToRun, Class<T> type) {
		ExecutorService threadPool = Executors.newFixedThreadPool(count);
		ArrayList<T> nets = new ArrayList<T>(thingsToRun.length);
		Set<Future<T>> futures = new HashSet<Future<T>>(thingsToRun.length);
	...
	public static void runMultipleNeuron(TestPatterns testPatterns,
		...
		// once we have submitted all jobs to the thread pool, it should be
		// shutdown
		threadPool.shutdown();
	}
	 ...
	 */
	public static MultiLayerNet[] runPreTrain(TestPatterns pretrain,
			TestPatterns posttrain, int count, boolean verbose) {
		PretrainThreadNet[] mn = new PretrainThreadNet[count];
		for (int i = 0; i < count; ++i) {
			mn[i] = new PretrainThreadNet(null, pretrain, posttrain, 200, 30,
					40, i, true, verbose);
		}
	 ...
	 * @return the multi layer net[]
	 */
	protected static MultiLayerNet[] runPreTrainGradual(TestPatterns pretrain,
			TestPatterns posttrain, int count, boolean verbose) {
		PretrainGradualThreadNet[] mn = new PretrainGradualThreadNet[count];
		for (int i = 0; i < count; ++i) {
			mn[i] = new PretrainGradualThreadNet(null, pretrain, posttrain,
					100, 100, 50, i, verbose);
		}
		ArrayList<MultiLayerNet> aNets = MultiNNUtilities.runCallableThreads(
				count, mn, MultiLayerNet.class);
		return Committee.MultiLayerListToArray(aNets);
	...
	public static void testCommittee(TestPatterns testPatterns,
			Committee committee) {
		ConfusionMatrix cm = committee.testPatterns(testPatterns
				.getTrainingPatterns());
		cm.setVerbose(true);
