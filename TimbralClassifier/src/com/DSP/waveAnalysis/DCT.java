...
public class DCT extends TransformComponent {
	...
	/**
	 * Simplifies DCT by only using the negative component and flipping it into
	 * positive.
	 * 
	 * @param dctBox
	 *            the dct box
	...
	private static double[] newDCT(Complex[] fft) {
		...
		for (int i = 0; i < fft.length; ++i) {
			e = Math.pow(Math.E, -1d * (double) i * Math.PI
					/ (double) (fft.length * 2));
			// Log.d( + " " + fft[i].re);
			dct[i] = e * fft[i].re;
		}
	...
	public FFTBox analyse() {
		...
		double[] freqRow = ArrayMethods.extend(
				DCT.getFreqRow(signal, amps.length), amps.length / 2);
		amps = dct(amps);
	...
	public void makeGraph(int filterFrom, int filterTo, int width, int height) {
		...
		DCTController sc = new DCTController(FFTBox.logarithmicFreq(FFTBox
				.filter(fb, filterFrom, filterTo)), width, height);
		sc.makeChart();
	}
