...
/**
 * Class for determining the fundamental pitch of a waveform. DEPRECATED, USE
 * GETFUNDAMENTAL IN PITCH INSTEAD
 * 
 * @author Jonny Wildey
 * @deprecated
 */
...
public class WaveFund {
	...
	/**
	 * for fundamental period analysis principally. Finds largest, highest
	 * occurring number in a 2d array.
	 **/
	...
	/**
	 * when given an arraylist of sample indexes returns an array of the
	 * distance between one sample and the next.
	 */
	...
	private ArrayList<Integer> getSignChanges() {
		ArrayList<Integer> zeroX = new ArrayList<Integer>(frame); // can't be
		// more zero
		// crossing
		// than
		// samples
	...
	/**
	 * Looks in array for nearest (to the right) number. Uses both additive and
	 * percentage errors.
	 */
		...
		for (int j = 0; j < simDistance.length; ++j) {
			...
			for (int i = j + 1; i < zeroDif.length; ++i) {
				if (isWithinAddition(zeroDif[j], zeroDif[i], intError)
						| isWithinPercentage(zeroDif[j], zeroDif[i],
								percentageError)) {
					simDistance[j] = i - j;
					break;
				}
