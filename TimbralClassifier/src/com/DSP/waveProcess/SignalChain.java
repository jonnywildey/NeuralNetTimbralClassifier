...
public class SignalChain {
	...
	public static Signal addNoise(Signal s, Random nr, double maxLoud) {
		...
		try {
			// pick which
			if (nc < 0.3333) {
				Signal s2 = Gen.pinkNoise(s.getLength(), 0, s.getSampleRate(),
						s.getBit());
				s2 = Gain.volumeRMS(s2, loudness);
				s = Gain.sumMono(s, s2);
			} else if (nc < 0.6666) {
				Signal s2 = Gen.whiteNoise(s.getLength(), 0, s.getSampleRate(),
						s.getBit());
				s2 = Gain.volumeRMS(s2, loudness);
				s = Gain.sumMono(s, s2);
			} else {
				Signal s2 = Gen.tapeHiss(s.getLength(), 0, s.getSampleRate(),
						s.getBit());
				s2 = Gain.volumeRMS(s2, loudness);
				s = Gain.sumMono(s, s2);
			}
	...
	public static FFTBox basicPatternProcess(FrameFFT fft) {
	 ...
	 * @return the signal
	 */
	public static Signal processSignalChain(Random pitchRand, Random noiseRand,
			Random hpRand, Random lpRand, Signal s1) {
		s1 = Gain.getMid(s1);
