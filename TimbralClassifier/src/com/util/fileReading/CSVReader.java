...
public class CSVReader {
    ...
    /*So I had this totally sweet idea of how to determine what format we want the data in. 
     * We use an INDEX!!!! 0 is string, 1 is int, 2 is double, 3 is float
     */
    ...
    private double stringToDouble(String str) {
 	...
 	public CSVReader(String fname)
	    	...
	    	while (eof() != true) {
	    		...
	    		for (int i = 0; i < ss.length(); ++i) {
	    			...
	    			//char is an except (normally quotes)
	    				...
	    				//I would avoid looking at this method if possible. Is horrible
	    		...
	    		bufferArray.add(new ArrayList<String>(sArray.subList(currentPoint, sArray.size())));
   	...
   	public String[][] makeStringArray() {
		   ...
		   for (int i = 0; i < bufferArray.size(); ++i) {
			   stringArray[i] = bufferArray.get(i).toArray(new String[bufferArray.get(i).size()]);
		   }
    	...
    	private CSVValue readQuotes(String ss) {
			...
			//if qcount > 0 we have reached the end of a bunch of quotes and need to write them
			else if (qcount > 0) {
									for (int i = 0; i < qcount / 2; ++i) {
										vb.append(except);
										}
