...
/**
 * Basic RIFF type chunk object. In order to stop the whole thing being horribly
 * confusing most methods just read/write straight from the chunk byte array.
 * However this can't really be done for sub-chunks, so be aware that every time
 * you make a sub chunk you are essentially copying the data. As this will be
 * mainly used for meta data I don't think this will be much of a problem *
 * 
 * @author Jonny Wildey
 * @version 1.0
 */
...
public class Chunk implements Serializable {
	...
	protected static int dataOffset = 8; // amount of bytes before actual data.
	// Normally 8
	...
	protected long getSubChunkSize(Integer ic) {
		return HexByte.hexToDecimalUnsignedLE(HexByte.getOffsetSubset(
				this.bytes, ic + 4, 4));
	}

	/**
	 * Does this object actually have any information or is it an empty shell?
	 */
	...
	/**
	 * Does this chunk contain a chunk with name? Returns the position of chunk
	 * if yes and null if not
	 **/
	...
	/**
	 * kind of lame method that determines which class should be made. length
	 * input is PRE offset
	 **/
	...
	public String toString() {
		...
		if (hasBytes()) {
			...
			sb.append("\tData: "
					+ new String(ArrayMethods.getSubset(bytes, dataOffset)));
		}
		return sb.toString();
	}
